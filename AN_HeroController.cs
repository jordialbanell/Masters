...
public class AN_HeroController : MonoBehaviour
{
    ...
    void Update()
    {
        MyPlayerPosition = GameObject.Find("Player").transform.position;
        scene = locate();
        // camera rotation
    ...
    public int locate()
    {
      if ((MyPlayerPosition[0] <= 356 && MyPlayerPosition[0] >= 352) &&
      (MyPlayerPosition[1] <= 3 && MyPlayerPosition[1] >= 2) &&
      (MyPlayerPosition[2] <= 91 && MyPlayerPosition[2] >= 80))
      {
        print("Build a Song");
        return (1);

      } else if ((MyPlayerPosition[0] <= 356 && MyPlayerPosition[0] >= 352) &&
      (MyPlayerPosition[1] <= 3 && MyPlayerPosition[1] >= 2) &&
      (MyPlayerPosition[2] <= 125 && MyPlayerPosition[2] >= 124))
      {
        print("Match the Sound");
        return (2);
      }
